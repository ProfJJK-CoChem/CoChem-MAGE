# CoChem-MAGE: Machine Learning Force Fields (MLFF)
...
Equivariant neural network inference engine.
...
Supports AIMNet2, MACE-OFF23, and ANI-2x. Evaluates out-of-distribution (OOD) errors using Mahalanobis distance profiling against the training set covariance matrix.
...
```bash
cochem-mage --model mace-off23 --input mol.xyz
```
...
pipeline.run(target_module="CoChem-MAGE")
```
